...
if polynomial_option == 0 or polynomial_option == 1 or polynomial_option == 2:
 ...
 xVelocityAux2BC = np.zeros([numNodes,1], dtype = float)
 ...
 yVelocityAux2BC = np.zeros([numNodes,1], dtype = float)
 ...
 pressureAux2BC = np.zeros([numVerts,1], dtype = float)
...
for t in tqdm(range(1, nt)):
 ...
 try:
  ...
  b = b + dirichletVector
  print '1 ok'
  sol = scipy.sparse.linalg.cg(A,b, sol, maxiter=1.0e+05, tol=1.0e-05)
  print '2 ok'
